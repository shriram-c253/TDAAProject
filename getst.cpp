...
std::map<int, std::map<int, double>> get_wtd_graph(std::map<int, std::set<int>> &graph, std::map<std::pair<int, int>, double> &weights)
{
...
//Returns a MST by Kruskal's algorithm for the starting for of delta*+1 algorithm
std::map<int, std::set<int>>
get_spanning_tree_wtd(std::map<int, std::set<int>> &graph, std::map<std::pair<int, int>, double> &weights)
{
	std::map<int, std::map<int, double>> wtd_std = kruskals(get_wtd_graph(graph, weights)).second;
