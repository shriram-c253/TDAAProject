...
bool comp::operator()(std::pair<double, std::pair<int, int>> a,
                      std::pair<double, std::pair<int, int>> b) {
  return a.first > b.first;
}

std::pair<double, std::map<int, std::map<int, double>>> *
bdd_prims(std::map<int, std::map<int, double>> graph, int d_bound, int src) {
  std::priority_queue<std::pair<double, std::pair<int, int>>,
                      std::vector<std::pair<double, std::pair<int, int>>>, comp>
      mpq;
  ...
  while (done < n) {
    ...
    mpq.pop();
    double w = curr_v_d.first, v = curr_v_d.second.second,
           u = curr_v_d.second.first;
    ...
    for (std::pair<int, double> e : graph[v]) {
      ...
      if (sp_tree.find(v1) != sp_tree.end())
        continue;
      mpq.push({w1, {v, v1}});
    }
    done++;
  ...
  return new std::pair<double, std::map<int, std::map<int, double>>>(totcost,
                                                                     sp_tree);
}
