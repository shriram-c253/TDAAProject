 ...
 * and also tries to reduce the weight of the formed spanning tree to as much
 * extent as possible
 */
...
delta_star_plus_one_wtd(std::map<int, std::set<int>> graph,
                        ...
                        int d_bound) {
  // Heuristic 1 :
  // get a spanning tree for the graph by running a minimum weight spanning tree
  // algorithm This ensures that the spanning tree with which we start has the
  // smallest weight possible
  // TODO : This is not done yet
  std::map<int, std::set<int>> spanning_tree =
      get_spanning_tree_wtd(graph, weights);
  int k = 0;
  for (auto &e : spanning_tree) {
    k = std::max(k, (int)e.second.size());
  }

  // sorted weights will store the weights and the corresponding vertices in
  // increasing order of edge costs
  ...
  while (true) {
    // std::cerr << "TOTWT " << tot_weight << std::endl;
    if (k <= d_bound) {
      double check_wt = 0;
      int maxdeg = 0;
      for (auto &x : spanning_tree) {
        maxdeg = std::max(maxdeg, int(x.second.size()));
        for (auto &y : x.second) {
          check_wt += weights[{x.first, y}];
        }
      }
      check_wt /= 2;
      // std::cerr << "BRUH " << check_wt << ' ' << tot_weight << ' ' << maxdeg
      // << std::endl;

      return new std::map<int, std::set<int>>(spanning_tree);
    }
    ...
    std::map<int, double>
        benefit; // this stores for every degree k vertex that is made good, the
                 // cost of the maximum weight edge incident to it in the cycle
                 // we form adding an edge to the spanning tree

    // Heuristic 2 :
    // our objective is to find the minimum weight edge that joins 2 forests, so
    // that cost of the new spanning tree on adding this edge is the smallest
    // possible
      ...
      if (d.findParent(from) != d.findParent(to)) {
        ...
        for (auto &vertex : cycle) {
          ...
          if (cycle_pairs.find(vertex) == cycle_pairs.end()) {
            cycle_pairs[vertex] = {
                from, to}; // setting cycle pair for a vertex for which we
        ...
        for (int i = 0; i < (int)cycle.size() - 1; i++) {
          ...
          if ((int)spanning_tree[vertex].size() == k) {
            ...
            int next = cycle[i + 1];
            benefit[cycle[i]] =
                std::max(weights[{vertex, prev}], weights[{vertex, next}]);
          }
        }
      }
    }
